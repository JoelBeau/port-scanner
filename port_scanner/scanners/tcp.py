...
class TCPConnect(Scan):
    ...
    async def _scan_batch_connect(
        ...
        logger_message = f"Starting TCP Connect scan on host {self._host} for ports {self._ports}."
        ...
        if self._ports[-1] > conf.THRESHOLD_FOR_SLOW_SCAN:
            logger_message = f"Scanning more than {conf.THRESHOLD_FOR_SLOW_SCAN} ports with tcp connect may be slow."
        ...
        async def scan_port(port: int):
            ...
            for attempt in range(self._retry + 1):
                logger_message = f"Scanning port {port} on host {self._host}, attempt {attempt + 1}..."
                logger.info(logger_message)
                ...
                if attempt > 0:
                    logger_message = f"Retrying port {port} on host {self._host}, attempt {attempt + 1}..."
                    logger.warning(logger_message)
